...
public class GeografijaDAO {
    ...
    public void dodajDrzavu(Drzava drzava) {
        try {
            ...
            if (drzava.getGlavni_grad() != null) ps.setInt(2, drzava.getGlavni_grad().getId());
            else {
    ...
    public ArrayList<Drzava> drzave()
    {
        ...
        try{
            String upit="SELECT d.id, d.naziv, d.glavni_grad AS grad FROM Drzava d JOIN Grad g ON g.drzava=d.id ORDER BY g.broj_stanovnika DESC";
            PreparedStatement ps=konekcija.prepareStatement(upit);
            ResultSet rs=ps.executeQuery();
            ...
            while(rs.next())
            {
                ...
                drz.setId(rs.getInt("id"));
                drz.setNaziv(rs.getString("naziv"));

                Grad grad=new Grad();
                grad.setNaziv(rs.getString("grad"));
                //grad.setBroj_stanovnika(rs.getInt("broj_stanovnika"));

                drz.setGlavni_grad(grad);
    ...
    public void obrisiGrad(Grad grad) {
        try {
            String upit = "DELETE FROM Grad WHERE id = ?";
            PreparedStatement ps = konekcija.prepareStatement(upit);
            ps.setInt(1, grad.getId());

            int promijenjeniRedovi = ps.executeUpdate();
            if (promijenjeniRedovi == 0) {
                throw new SQLException("Brisanje grada nije uspjelo");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
